...
def render_variant(run_config, spec, variant_name, properties):
    ...
    for obj_spec in spec["variants"][variant_name]["objects"]:
        objects.append(
            add_controlled_object(
                obj_spec,
                properties,
                run_config["shape_dir"],
                camera,
            )
        )

    preferred = spec["preferred"][variant_name]
    target_count = spec["variants"][variant_name]["target_count"]
...
def main(argv=None):
    ...
    print("COUNT_PILOT_V1_RENDER_OK")
    sys.stdout.flush()
...
if __name__ == "__main__":
    if not INSIDE_BLENDER:
        ...
        print("blender --background --python clevr_count_renderer.py -- --run_config ... --specs_file ...")
        sys.exit(1)
